...
export const getLocation = state => id => {
  const foundLoc = state.general.locations.find(location => location.id === id);
  return foundLoc;
};
...
export const getApplicantsFiltered = state => type => {
  ...
  let nwlist;
  if (type === "rostered") {
    nwlist = state.job_applicants.applicants.filter(el => el.rostered === true);
    ...
    nwlist = state.job_applicants.applicants.filter(el => el.blocked === true);
